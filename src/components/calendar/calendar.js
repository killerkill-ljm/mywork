export default {
    ...
    // 向前空几个
    getPreMonthRemain(date) {
        const year = date.getFullYear();
        const month = date.getMonth() + 1;
        const dateFirstOne = new Date(year + '/' + month + '/1');
        return this.sundayStart ?
            dateFirstOne.getDay() == 0 ? 7 : dateFirstOne.getDay() ://六行
            dateFirstOne.getDay() == 0 ? 6 : dateFirstOne.getDay() - 1;//五行
    },
    /**
     * 获取当前日期上个月或者下个月
    */
    ...
    getLeftArr(date) {
        const arr = [];
        const leftNum = this.getPreMonthRemain(date);
    ...
    getRightArr(date) {
        const arr = [];
        const leftLength = this.getDaysInOneMonth(date) + this.getPreMonthRemain(date);
